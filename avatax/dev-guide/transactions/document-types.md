...
Because every type of transaction must be able to follow this same pattern, AvaTax supports all transaction types as both Orders and Invoices. It’s worth noting that the <code>Invoice</code> transaction type provides a key compatibility between Avalara’s tax calculation API and the Avalara Managed Returns Service. The Managed Returns Service only supports filing taxes calculated with AvaTax - but you can only file taxes that were recorded in <code>Invoice</code> types! Anything you calculated using <code>Order</code> types is considered a temporary estimate and won’t be reported.
