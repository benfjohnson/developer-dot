 ...
 */
const updateDataByPropertyPath = (propertyPath, newValue, data) => {
...
export default (state, action) => {
    ...
    case actionTypes.INPUT_CHANGED:
        ...
        if (input.in !== 'postBody') {
            const newQsOrPath = {...recipe.request[input.in], [input.field]: action.newValue};

            newRequest = {...recipe.request, [input.in]: newQsOrPath};
        } else {
            newRequest = {...recipe.request, postBody: updateDataByPropertyPath(input.field, action.newValue, recipe.request.postBody)};
        }
