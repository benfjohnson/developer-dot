...
As you can see from the lifecycle document above, a transaction can go through a number of steps before it is finalized.  We have designed these steps to be flexible enough to solve problems for a variety of different customers and different types of tax processes.  Clearly, the way you use AvaTax will be different Let's talk through a few common ones.

<h3>Scenario 1: Online Storefront</h3>

In an online store, your first task is to provide a sales tax estimate so that a user casually browsing through your website can easily see what their total payment will be.  These casual visitors have not committed to buy anything - but you can still give them accurate tax information.  To solve this problem, you call <a href="https://sandbox-rest.avatax.com/swagger/ui/index.html#!/Transactions/CreateTransaction">CreateTransaction</a> with the transaction type set to `SalesOrder`.  This causes the transaction to return correct results, but it won't record any taxes yet, because the transaction is not finalized.

When the customer chooses to finish their transaction, your storefront should call <a href="https://sandbox-rest.avatax.com/swagger/ui/index.html#!/Transactions/CreateTransaction">CreateTransaction</a> again, but this time set the transaction type to `SalesInvoice`.  This causes the transaction to be record in AvaTax, so it can be consulted later.  The reason you have to contact the API a second time may not be immediately obvious - but the customer may have waited long enough that the tax rates might have changed, or their address may have changed, or your company configuration may have changed.  Any one of these small changes can affect the accuracy of a tax calculation; so when you actually execute the transaction you have to make this second API call.
...
<ul class="normal">
    ...
    <li>Charlie works in the warehouse, and she reviews the transactions a second time prior to fulfillment.  She makes the final decision whether to accept, reject, or adjust transactions.  If the transaction is accepted, Charlie calls <a href="https://sandbox-rest.avatax.com/swagger/ui/index.html#!/Transactions/CommitTransaction">CommitTransaction</a>, and if the transaction should be voided, she calls <a href="https://sandbox-rest.avatax.com/swagger/ui/index.html#!/Transactions/VoidTransaction">VoidTransaction</a>.</li>
</ul>

You can easily bypass one of these three steps if you would like to go directly from `Saved` to `Committed`.  Just keep in mind that Avalara's Managed Returns service will file any transactions unless they are `Committed`, so please make sure that the user responsible for committing transactions understands this behavior.
...
In many accounting systems, a transaction is considered a permanent record that cannot be modified.  In this approach you should consider all transactions as committed immediately, and you should allow the original transaction to be committed, plus you should add the `ReturnInvoice` transaction.  This approach better matches the activity of your business, therefore it's generally considered best practice.
